...
#Bdellovibrio (foxes)
f_init = int(0.5*num_agents) #initial Bdellovibrio population

mf = 0.05 # magnitude of movement of Bdellovibrio
df = 0.05 # death rate of Bdellovibrio
hunting_f = 0.5  # probabilidad de caza fortuita
...
#Pseudomonas (rats)
r_init = int(0.5*num_agents) # 0.7 initial Pseudomonas population

rr = 0.15 # old reproduction rate of Pseudomonas (1/tiempo medio de reproducción)
...
dr=0.08#2 # probabilidad de muerte por unidad de tiempo
...
def draw_evol2():
    ...
    if radata[0]!=0:
        pp.plot(radata,'g',label = '$\it{Presas 2}$')      # cada vez que se ejecutó evolt()
    
    if hdata[0]!=0:
        pp.plot(hdata,'c',label = '$\it{Presas 3}$')
    
    pp.plot(fdata,'r',label = '$\it{Depredadores}$')
    
    if wdata[0]!=0:
        pp.plot(wdata,'y',label = '$\it{Depredadores 2}$')
...
def update():
    ...
    # Depredación, reproducción y adaptación de hunting
    
        for ag in agents:
            
            if ag.type=='f':
                
                recogida_off.append(ag.off)
                ...
                if len(neighbours)>0:
                    ...
                    # ajustamos el offspring ya que se ha topado con una bacteria    
                        
                    neigh_dep = [nbd for nbd in agents if (nbd.type == 'f') and (ag.x - nbd.x)**2 + (ag.y - nbd.y)**2 < 4*cdsq]
                    ...
                    if random() < hunting_f:     # aplicamos probabilidad de caza fortuita
                        ...
                        for i in range(ag.off): # creamos los nuevos agentes
                ...
                if len(neighbours)>0:
                    ...
                    neigh_dep = [nbd for nbd in agents if (nbd.type == 'w') and (ag.x - nbd.x)**2 + (ag.y - nbd.y)**2 < 4*cdsq]
        ...
        control_offspring.append(sum(recogida_off)/len(recogida_off))                    
...
draw_cycle()
#pp.savefig('Adaptativo_con hunting_=_{0:.3f},_df_=_{1:.3f},_rr_=_{2:.3f}_dr_={3:.3f}.png'.format(hunting_f,df,rr,dr))
pp.show()

array_pres=array(rdata[300:len(rdata)-1])
media_pres=sum(array_pres)/len(array_pres)
disp_pres=(sum((array_pres-media_pres)**2)/len(array_pres))**0.5

array_dep=array(fdata[300:len(fdata)-1])
media_dep=sum(array_dep)/len(array_dep)
disp_dep=(sum((array_dep-media_dep)**2)/len(array_dep))**0.5

medias=[media_pres,media_dep]
print(medias)

dispersiones=[disp_pres,disp_dep]
print(dispersiones)

array_dif = array(fdata[300:len(fdata)-1])-array(rdata[300:len(rdata)-1])
media_dif = sum(array_dif)/len(array_dif)
disp_dif=(sum((array_dif-media_dif)**2)/len(array_dif))**0.5

print('media diferencia = ',media_dif)
print('dispersion diferencia = ',disp_dif)

print('hunting = ',hunting_f)
print('df = ',df)

pp.plot(control_offspring)
pp.xlabel('Evolution time (a.u.)')
pp.ylabel('Media del parámetro offspring')
pp.show()

win32api.MessageBox(0, '¡Ya acabé!', 'adaptación offspring', 0x00001000) 
